...
int balance = 0;
	...
	/*@ 
	 ...
	 private boolean  update__wrappee__CreditWorthiness  (int x) {
	...
	boolean update(int x) {
		...
		if (!update__wrappee__CreditWorthiness(x))
			return false;
	...
	/*@ 
	 ensures (!\result ==> balance == \old(balance)) 
	  && (\result ==> balance == \old(balance) - x); @*/
	 private boolean  undoUpdate__wrappee__CreditWorthiness  (int x) {
		int newBalance = balance - x;
		if (newBalance < OVERDRAFT_LIMIT)
			return false;
		balance = newBalance;
	...
	boolean undoUpdate(int x) {
		...
		if (!undoUpdate__wrappee__CreditWorthiness(x))
			return false;
		withdraw = newWithdraw;
		return true;
	}
