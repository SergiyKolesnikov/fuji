...
public class MailContentPane extends JSplitPane implements ListSelectionListener, ActionListener{
	...
	public void setProgressBar( JProgressBar bar){
		((MailContentPane) this).pbStatusbar = bar;
	}
    ...
    private void createPane(){
		...
		table.getSelectionModel().addListSelectionListener(((MailContentPane) this));
		...
		((MailContentPane) this).setTopComponent(tablePanel);
		...
		((MailContentPane) this).setBottomComponent(contentPanel);
		((MailContentPane) this).setDividerLocation(0.5);
		((MailContentPane) this).validate();
    }
    ...
    public void insertMessages(){
		...
		((MailContentPane) this).pbStatusbar.setMinimum(0);
		((MailContentPane) this).pbStatusbar.setMaximum(messageCount);
		...
		for (int i=1; i<=messageCount; i++){
			...
			((MailContentPane) this).pbStatusbar.setValue(i);
			((MailContentPane) this).pbStatusbar.paintImmediately(((MailContentPane) this).pbStatusbar.getVisibleRect());
    ...
    public void insertContent(){
		...
		try{
			...
			button.setIcon(icon);
			button.setText(text);
			button.addActionListener(((MailContentPane) this));
			images.add(button);
			buttons.add(button);
			((MailContentPane) this).validate();
		    }
	...
	public void refresh(){
		...
		if (messageCount > table.getRowCount()){
			((MailContentPane) this).pbStatusbar.setMinimum(0);
			((MailContentPane) this).pbStatusbar.setMaximum(messageCount - table.getRowCount());
		...
		if(source instanceof JButton){
		    for(int i=0; i<buttons.size(); i++){
				if(buttons.get(i) == source){
				    ...
				    int result = fc.showDialog(((MailContentPane) this),"Save");
