 ...
 */
abstract class Server$$Core implements ChatComponent {
	 ...
	 *            port to listen on
	 */
	public Server$$Core(int port) throws IOException {
		try {
			intialize( port);
		} catch (Exception e) {
			e.printStackTrace();
...
public class Server extends  Server$$Core  {
	public void intialize(int port) throws Exception {
		System.out.println("Logger START!");
		
		/*if[LOG]*/ 
		new LogWriter(((Server) this));
		/*end[LOG]*/
		
		super.intialize(port);
